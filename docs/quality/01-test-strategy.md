...
flowchart TB
    ...
    unit["Unit-Tests (Repos, Validierung, kleine Helfer)"]
...
| Performance | `pytest -m performance --no-cov` |
| End-to-End Sync | `backend/tests/test_sync_hybrid_e2e.py` |

## CI-Erwartungen

- Lint und Format gruen.
- Test-Coverage mit Mindestschwelle (im Desktop aktuell 30 % fuer
  Kernmodule).
- Security-Scans aktualisiert.
- Stability/Acceptance-Suite gruen.
...
([Acceptance-Suite](02-acceptance-suite.md)).
...
```bash
# Hybrid-Sync E2E (Desktop + ndhub-web als Geschwister)
PYTHONPATH="ndhub-web:desktop-client" python3 -m pytest \
  ndhub-web/backend/tests/test_sync_hybrid_e2e.py
```
...
- Performance-Suiten laufen nicht standardmaessig in CI; sie sind als
  manueller Workflow vorgesehen.
