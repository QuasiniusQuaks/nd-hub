...
class CompletionRing(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.progress > 0:
            color = c['blue']
            if self.progress >= 100: color = c['status_green']
...
class CollapsibleSection(QtWidgets.QWidget):
    ...
    def _on_header_clicked(self, event):
        ...
        v_icon = IconManager.get_icon("chevron_down", color=AppleTheme.current_colors()['blue'])
        r_icon = IconManager.get_icon("chevron_right", color=AppleTheme.current_colors()['blue'])
...
class AppleDashboard(ResponsiveWidget):
        ...
        if hasattr(self, 'tracking_cards_layout'):
            for i in range(self.tracking_cards_layout.count()):
                ...
                if item and item.widget():
                    w = item.widget()
                    if hasattr(w, 'badge_moves'): w.badge_moves.update_style()
                    if hasattr(w, 'badge_stock'): w.badge_stock.update_style()
                    w.update()
    ...
    def _create_kpi_grid(self):
        ...
        bewegungen_query = """
            ...
            WHERE strftime('%Y-%m', COALESCE(eingangsdatum, ausgangsdatum)) = ?
        """
        try:
    ...
    def refresh_tracking(self):
        ...
        for row in depots:
            ...
            if moves: reached_steps += 1
            if stock: reached_steps += 1
            if moves and stock: completed_count += 1
